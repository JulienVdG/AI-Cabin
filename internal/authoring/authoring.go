...
func Assemble(bps []fragments.BundleBlueprint, h cabin.AICabinHeader, w *Files) error {
	...
	var errs []error
	if w.Dockerfile != nil {
		if e := m.writeDockerfile(w.Dockerfile, h); e != nil {
			errs = append(errs, e)
		}
	}
	if w.Compose != nil {
		if e := m.writeCompose(w.Compose, h); e != nil {
			errs = append(errs, e)
		}
	}
	if w.Taskfile != nil {
		if e := m.writeTaskfile(w.Taskfile, h); e != nil {
			errs = append(errs, e)
		}
	}
...
func (m MergedBlueprint) writeCompose(w io.Writer, h cabin.AICabinHeader) error {
	...
	build.Content = append(build.Content,
		...
		strNode("args"), seqNode([]string{"HOST_UID=${HOST_UID:-1000}"}),
	)
	service.Content = append(service.Content,
		strNode("build"), build,
		strNode("image"), strNode(h.Cabin),
		strNode("hostname"), strNode(h.Cabin),
	)
	if m.Compose != nil {
...
func (m MergedBlueprint) writeTaskfile(w io.Writer, h cabin.AICabinHeader) error {
	root := &yaml.Node{Kind: yaml.MappingNode, Tag: "!!map"}

	header := &yaml.Node{}
	if err := header.Encode(h); err != nil {
		return fmt.Errorf("encode ai-cabin header: %w", err)
	}
