...
I analyzed `converter.go`. Its philosophy is: single-pass conversion with minimal allocations.

I re-read: 
- `desk/DEVELOPMENT.md` - Conversion patterns
- `src/converter_test.go` - Test cases

I propose: Two-pass approach (collect → process) to avoid iteration issues during node movement.

⚠️ Attention point: This deviates from current single-pass pattern because it requires additional memory.
