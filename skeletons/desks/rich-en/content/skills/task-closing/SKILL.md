...
```markdown
- [x] **Task name** (Done ✅) : Summary description of what was done.
```
...
```markdown
- [x] **Refactor validation layer** (Done ✅) : Validation API extracted to separate package with public `validator.New()` function. 23 tests passing.
```
...
- [x] **Converter edge case fix** (Done ✅) : Handle empty input and nil pointer cases. 23 tests passing.
...
- [x] **Validation layer** (Done ✅) : Validator package with functional options pattern. Public API `validator.New()`. 23 tests passing.
...
```markdown
- [ ] **User's feature request** (TODO 🚩) : Reformulated description.
```
...
- [ ] **Empty input handling** (TODO 🚩) : Add validation for empty/nil input in converter.
